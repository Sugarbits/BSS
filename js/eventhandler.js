function eventhandler_history(){
console.log("eventhandler_history");
var JSONdata = JSON.parse(datadeal_listbatchSelect());
	formdeal_historyfilled(JSONdata,'history');
	//http://stackoverflow.com/questions/10373618/jquerymobile-error-cannot-call-methods-on-listview-prior-to-initialization
}

//給 history 小夥伴的「每人一個」小眼睛
function eventhandler_history_sidebtn(id){
	 $( "#"+id ).on("click",(function() {
	 var _batchId = parseInt((id).split("-")[2]);
	 var JSONdata = JSON.parse(datadeal_batchitemSelect(_batchId));
	 formdeal_historydetailfilled(JSONdata,'historydetail');
		//alert(_batchId);
...
//給 history 小夥伴的小眼睛
function eventhandler_historypopup(id){
	  $( "#"+id ).bind({
       popupafteropen: function(event, ui) { 
				//console.log('history 開眼');
       },
       popupafterclose: function(event, ui) { 
			formdeal_historydetailclean('historydetail-1');
			formdeal_historydetailclean('historydetail-2');
			formdeal_historydetailclean('historydetail-3');
			formdeal_historydetailclean('historydetail-4');
				// console.log('history 閉眼');
       }
    });
...
function eventhandler_radio(){
	...
	//alert(_var);
        if (_var == '') {
            ...
            //alert(":"+id+":"+_var);
			formdeal_optionfilled(JSON.parse(datadeal_listtypeSelect()),id,_var);
        }
		})
    );
}
...
function eventhandler_tab2_popup2button1(){
	...
	//alert('Y');
	var batchId = datadeal_batchInsert($('#text-2-name').val(),'1970-1-1',0);
	if(batchId > 0){//-1 ,-2 是錯誤狀態 0暫時保留給斷網	
		var obj = (datadeal_invert_checkbox2get('checkbox-2'));
		var send_result = datadeal_itemInsert(JSON.stringify(obj),batchId);
		if(0==send_result){
			alert('送出成功!');
...
function eventhandler_popupsend(id){
	  $( "#"+id ).bind({
       popupafteropen: function(event, ui) { 
				...
				var _date_write = (date.getFullYear()).toString()+(date.getMonth()+1).toString()+(date.getDate()).toString();
				$( "#text-2-date" ).val(_date);
				$( "#text-2-date" ).attr('readonly',true);
				$( "#text-2-name" ).val('新增清單'+_date_write);
...
function eventhandler_popupadd(id){
	  $( "#"+id ).bind({
       popupafterclose: function(event, ui) { 
         $("#text-amount-1").val('');
       }
